...
export async function analyzeWithGemini(formData: FormData): Promise<string> {
  try {
    const prompt = `
    ...
    const response = cleanAndFormatResponse(result.response.text());

    return response;
  } catch (error: unknown) {
    console.error("Error calling Gemini API:", error);
    const errorMessage = error instanceof Error ? error.message : String(error);
    return `<p class="text-red-500">שגיאה בקבלת ניתוח מ-Gemini: ${errorMessage}</p>`;
  }
}
